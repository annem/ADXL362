...
Will "wake up" when circuit is turned upside down.
...
void setup(){
    ...
    xl.setupDCActivityInterrupt(1000, 100);
    xl.setupDCInactivityInterrupt(975, 100);
	Serial.println();
    
	
    //  Setup ADXL362 for proper autosleep mode
    //  STILL NEEDS CLEANUP OF COMMENTING
    xl.SPIwriteOneRegister(0x27, 0x35); // link, loop, inact, act
